namespace PeripassGame.Domain.Tests;
...
public class WordCombinationFinderTests(ITestOutputHelper _output) {
  ...
  public async Task GetAllCombinations_SingleSixLetterWordWithTwoPartials_FindsSingleMatch() {
    ...
    Assert.Equal("foo+bar=foobar", result[0]);
  }
  ...
  public async Task GetAllCombinations_MultiplePartialsFormingMultiplePairs_FindsAllMatches() {
    ...
    Assert.Contains("fo+o+bar=foobar", result);
    Assert.Contains("foo+bar=foobar", result);
    Assert.Contains("fo+o+baz=foobaz", result);
    Assert.Contains("foo+baz=foobaz", result);
  }
  ...
  public async Task GetAllCombinations_LargeHardcodedDataset_ConsistentResultsAndLogsExecutionTime() {
    ...
    // 3 dry runs should warm everything up (JIT, caches, etc.) before we start measuring
    _ = await finder.GetAllCombinationsAsync(options);
    _ = await finder.GetAllCombinationsAsync(options);
    _ = await finder.GetAllCombinationsAsync(options);
    ...
    for (var i = 0; i < runs; i++) {
      var result = await finder.GetAllCombinationsAsync(options);
    ...
    //version one: Total execution time for 20000 runs: 9081 ms
    _output.WriteLine($"Total execution time for {runs} runs: {totalMs} ms");
  }

  private static readonly string[] LargeTestDataset =
  [
      "osine",
        "them",
        "narro",
